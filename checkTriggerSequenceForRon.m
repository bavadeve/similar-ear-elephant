%% 
facehouseMats = dir(['**' filesep '*facehouse*.mat']);
bdfs = dir(['**' filesep '*.bdf']);

female = [131:10:181 nan(1,6)];
male = [132:10:182 nan(1,6)];
house = 133:10:243;

fprintf('%1.0f bdfs found \n', length(bdfs))
fprintf('%1.0f trigger sequences found \n', length(facehouseMats))

for i = 1:length(bdfs)
    disp(bdfs(i).name)
    FILENAME = [bdfs(i).folder filesep bdfs(i).name];
    ...
    vals = [event.value];
    counter = 0;
    for j = 1:length(facehouseMats)
        load([facehouseMats(j).folder filesep facehouseMats(j).name], 'seq')
        seq2 = [];
        for k = 1:length(vals)
            if ismember(vals(k), female)
                seq2 = [seq2, 1];
            elseif ismember(vals(k), male)
                seq2 = [seq2, 2];
            elseif ismember(vals(k), house)
                seq2 = [seq2, 3];
            end
        end
        if all(seq==seq2)
            counter = counter + 1;
            fprintf('\t %s fits!\n', facehouseMats(j).name)
        end
    end
    if counter == 0
        fprintf('\t no fitting trigger sequence found, bdf missing?')
    elseif counter > 1
        fprintf('\t multiple trigger sequences fit, double subject?')
    end
end
