...
lims = struct;
lims.jump = 0; % set to zero if you want to remove jumps from data (time-intensive analysis)
lims.abs = 250; % uV
...
OPTIONS.RMCHANNELS.artfctdefStr    = 'ARTFCTBEFORE';
...
OPTIONS.WPLICONNECTIVITY.outputName   = ['WPLI' num2str(OPTIONS.WPLICONNECTIVITY.triallength)]; %'string': addition to filename when saving, so that the output filename becomes [currSubject outputName .mat] 
OPTIONS.WPLICONNECTIVITY.saveData    = OPTIONS.saveData; 
OPTIONS.WPLICONNECTIVITY.optionsFcn  = OPTIONS.pathsScript;
OPTIONS.WPLICONNECTIVITY.keeptrials  = 'yes';
...
OPTIONS.PLICONNECTIVITY.outputName   = ['PLI' num2str(OPTIONS.triallength) '_v2']; %'string': addition to filename when saving, so that the output filename becomes [currSubject outputName .mat] 
